...
class _BluetoothDeviceListState extends State<BluetoothDeviceList> {
  ...
  void _connectToDeviceIfAvailable() {
    if (devices.isNotEmpty) {
      _connectToDevice(devices.first);
    } else {
      print('Không tìm thấy thiết bị Bluetooth.');
      // Thực hiện xử lý tương ứng khi không tìm thấy thiết bị Bluetooth.
    }
  }
  ...
  void _sendControlCommand(String command) {
    print('Sent: $command');
    sendCommandViaBluetooth(connection, command);
  }
...
void sendCommandViaBluetooth(BluetoothConnection? connection, String command) {
  if (connection != null && connection.isConnected) {
    try {
    ...
    // Xử lý khi kết nối Bluetooth không khả dụng.
  }
}
